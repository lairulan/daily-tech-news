...
# Daily Tech News Publisher V4.3.1

每日 AI/科技/财经资讯自动策展与发布系统。

> 当前实际链路（2026-03-31）：仅使用 RSS 真实新闻源；入选新闻会补充原文上下文后再改写成单行新闻简讯；不再生成每条新闻下方的补充长文；云端定时仅保留 GitHub Actions。

## 功能概述

- 📰 **48 个 RSS 源** - AI/科技/财经三大领域全覆盖，100% 真实新闻
- 🔄 **RSSHub 多实例 fallback** - 中文财经源高可用，3 个镜像自动切换
- 🤖 **AI 智能整理** - 分类后补抓原文上下文，再改写为事实型新闻简讯
- 📝 **单行新闻简讯** - 每条新闻只保留一行 22-52 字的列表简讯
- 🔍 **模糊去重** - Jaccard 字符相似度阈值 0.6（v4.2新增）
- 🧹 **HTML 清洗增强** - CDATA + unescape + content:encoded（v4.2新增）
- 🩺 **RSS 源健康摘要** - 每次运行写入 `raw_news_*.json`，记录空返回与 fallback 命中情况
- 🎨 **精美视觉设计** - 渐变色卡片 + AI封面图
- 📋 **质量检查清单** - 发布前自动验证
- 🛡️ **环境预检查** - 减少运行时错误
- ⏰ **云端单触发定时** - GitHub Actions 每天一次发布
...
┌─────────────────────────────────────────────────────────────┐
│ Step 1: RSS 新闻采集                                          │
│   • 48个 RSS 源 • 24h 时间窗口 • fallback • 去重过滤           │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│ Step 2: AI 分类与简讯改写                                     │
│   • 三分类筛选 • 原文上下文补充 • 主体/时间校验               │
└─────────────────────────────────────────────────────────────┘
                              │
                              ▼
┌─────────────────────────────────────────────────────────────┐
│ Step 3: 内容生成与发布                                        │
│   • HTML 排版 • 微语/摘要 • 封面图 • 发布草稿箱                │
└─────────────────────────────────────────────────────────────┘
```
...
> 当前版本不使用热搜或 WebSearch 补充，只依赖 RSS 真实新闻源。

---

### Step 3: 新闻筛选与分类

使用 AI 将收集到的新闻整理成三个类别，并改写为单行新闻简讯：

1. **每类选择 5 条**最重要的新闻
2. **去重过滤**：基于标题相似度
3. **时间过滤**：仅保留昨天的新闻
4. **来源验证**：确保来自权威媒体
5. **原文补充**：抓取页面标题/导语，避免主体缺失
6. **简讯改写**：按“主体 + 动作 + 结果”生成单行事实简讯
...
### v4.3.1 (2026-03-31)
- 🧹 **硬过滤补强**：过滤 `派早报 / 观察 / 背后 / 盘点` 等不适合做新闻简讯的素材
- 🧠 **主体识别增强**：加入站点导航噪音清洗、主体候选打分和精确实体回填
- 🩺 **RSS 健康记录**：原始 JSON 追加 `rss_source_health`，便于追踪空返回与 fallback
- 📏 **标题容错优化**：单行简讯长度放宽到 22-52 字，同时保留主体、时间一致性校验

### v4.3.0 (2026-03-31)
- 🧭 **单行简讯化**：每条新闻只保留一行事实型新闻简讯，不再渲染下方补充长文
- 🔎 **原文上下文补充**：为入选新闻抓取页面标题/导语，降低主体缺失概率
- 🧱 **强约束校验**：新增主体与时间表达校验，防止“项目/模型/计划”等泛化主语和时间错乱
- ☁️ **云端单触发**：当前仅保留 GitHub Actions 定时发布
- 📝 **文档收口**：移除热搜/WebSearch 作为当前流程描述，统一到 RSS-only 链路
