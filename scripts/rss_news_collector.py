...
def validate_rewritten_title(item: Dict, subject: str, title: str) -> tuple:
    ...
    # 检测多项拼接：含分号说明是摘要列表而非单一标题
    if re.search(r"[;；]", title):
        return False, "标题含分号，疑似多条新闻拼接"

    # 检测 YYYY-MM-DD 日期格式混入标题
    if re.search(r"\d{4}-\d{2}-\d{2}", title):
        return False, "标题含完整日期格式，疑似摘要原文"

    # 检测多主体拼接：两个或以上中文公司/机构名之间仅靠空格分隔（无动词连接）
    # 特征：标题里有3个以上连续的词组以空格分隔且没有动词
    if len(re.findall(r"[\u4e00-\u9fff]{2,}(?:科技|集团|公司|汽车|智能|网络|云|AI|技术|平台|机器人|出行|医疗|金融|资本|传媒|教育|能源)", title)) >= 3:
        return False, "标题含3个以上机构名，疑似多条新闻拼接"

    # 检测语义不完整的并列/转折结构（只有前半句没有后半句）
    incomplete_patterns = [
        (r"要么[^，。,\.]{2,}$", "要么…要么…结构缺失后半句"),
        (r"(?:^|，)(?:虽然|尽管)[^，。]+$", "虽然/尽管缺失转折句"),
        (r"(?:^|，)(?:不仅|不但)[^，。]+$", "不仅/不但缺失后半句"),
        (r"(?:^|，)(?:一方面)[^，。]+$", "一方面缺失另一方面"),
    ]
    for pattern, reason_msg in incomplete_patterns:
        if re.search(pattern, title):
            return False, f"标题语义不完整: {reason_msg}"
...
7. 22-48字，单句，格式为”主体+动作+结果”，语义必须完整。
8. 禁止使用不完整的并列/转折结构：不能只写”要么A”而省略”要么B”；不能只写”虽然A”而省略”但B”；不能只写”不仅A”而省略”还B”。如材料本身就是并列结构，请改写为单一完整的事实陈述句。
...
4. 每条标题 22-42 字，单句，适合公众号新闻列表阅读，语义必须完整。
5. 标题格式统一为”主体 + 动作 + 结果”，避免”目前””当前””针对需要预测的事项””这类大模型”等空泛表达。
6. 仅允许保留品牌名/产品名的英文原文。
7. 禁止输出语义不完整的并列/转折句：不能只写”要么A”而漏掉”要么B”；不能只写”虽然A”而漏掉”但B”；不能只写”不仅A”而漏掉”还B”。遇到此类结构，请改写为一个独立完整的事实陈述句。
8. 严禁用分号（;/；）分隔多条新闻合并成一条——每条素材只能输出一句标题，描述一件事。
9. 严禁在标题中混入 YYYY-MM-DD 格式的日期。
10. 如果素材本身是摘要列表（含多家公司/多件事），只取其中最具体、新闻价值最高的一件事写标题，其余忽略。
8. 严禁用分号（;/；）分隔多条新闻合并成一条——每条素材只能输出一句标题，描述一件事。
9. 严禁在标题中混入 YYYY-MM-DD 格式的日期。
10. 如果素材本身是摘要列表（包含多家公司/多件事），只取其中最具体、新闻价值最高的那一件事写标题，其余忽略。
